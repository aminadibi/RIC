...
```
Relative Impact Characteristic (RIC) curve is a novel graphical tool that visualizes the power of a marker (e.g., a biomarker  or a risk prediction score) in terms of its capacity towards 'concentration of benefit' fro msubsequent therapies. 
...
outcome therefore corresponds to a line connecting points (0,0) and (1,1). If the relation
between marker values and treatment benefit is monotonic, the curve can take certain shapes
which are explained in figure below.
...
For each individual, the dataset contains the marker value (x), three covariates (c1-c3), a tratment assignment indicator (tx), log of follow-up time (ln_time), and number of events (events). The first few data rows are provided below

#Amin please rearrange the columns and also add a patient id column (left most)
